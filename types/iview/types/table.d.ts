...
export declare class Table extends Vue {
   ...
   * @default false
   */
  public 'disabled-hover'?: boolean;
  /**
   * 是否支持高亮选中的行，即单选
   * @default false
   */
  public 'highlight-row'?: boolean;
  /**
   * 行的 className 的回调方法，传入参数：
   * row：当前行数据
   * index：当前行的索引
   */
  public 'row-class-name'?: (row?: object, index?: number) => void;
  /**
   ...
   * @default 暂无数据
   */
  public 'no-data-text'?: string;
  /**
   * 筛选数据为空时显示的提示内容
   * @default 暂无筛选结果
   */
  public 'no-filtered-data-text'?: string;
  /**
   * slot插槽对象
   */
  public $slots: {
   ...
   */
  public $emit(eventName: 'on-current-change', currentRow: object, oldCurrentRow: object): this;
  /**
   * 在多选模式下有效，选中某一项时触发
   * selection：已选项数据
   * row：刚选择的项数据
   */
  public $emit(eventName: 'on-select', selection: object[], row: object): this;
  /**
   * 在多选模式下有效，取消选中某一项时触发
   * selection：已选项数据
   * row：取消选择的项数据
   */
  public $emit(eventName: 'on-select-cancel', selection: object[], row: object): this;
  /**
   * 在多选模式下有效，点击全选时触发
   * selection：已选项数据
   */
  public $emit(eventName: 'on-select-all', selection: object[]): this;
  /**
   * 在多选模式下有效，只要选中项发生变化时就会触发
   * selection：已选项数据
   */
  public $emit(eventName: 'on-selection-change', selection: object[]): this;
  /**
   * 排序时有效，当点击排序时触发
   * column：当前列数据
   * key：排序依据的指标
   ...
   */
  public $emit(eventName: 'on-sort-change', column?: object, key?: string, order?: 'asc' | 'desc'): this;
  /**
   * 筛选时有效，筛选条件发生变化时触发	当前列数据
   */
  public $emit(eventName: 'on-filter-change', value: any): this;
  /**
   * 单击某一行时触发
   * currentRow：当前行的数据
   * index?: 当前行的索引
   */
  public $emit(eventName: 'on-row-click', currentRow: object, index: number): this;
  /**
   * 双击某一行时触发
   * currentRow：当前行的数据
   * index?: 当前行的索引
   */
  public $emit(eventName: 'on-row-dblclick', currentRow: object, index: number): this;
  /**
   * 展开或收起某一行时触发
   * row：当前行的数据
   * status：当前的状态
   */
  public $emit(eventName: 'on-expand', row: object, status: string): this;
  /**
   * 导出数据
   */
  public exportCsv(params: TableExportCsvParams): void;
  /**
   * 执行改变大小重绘table
   */
...
export declare class TableColumn {
   ...
   */
  public type?: 'index' | 'selection' | 'expand' | 'html';
  /**
   * 列头显示文字
   * @default #
   */
  ...
  public render?: (h?:
    (
      ...
      data?: string | TableRenderCreateElementData | TableRenderCreateElementResult | TableRenderCreateElementResult[],
   ...
   */
  public sortMethod?: (a: any, b: any, type: 'asc' | 'desc') => void;
  /**
   ...
   */
  public filters?: Array<{ label: string, value: string | number | boolean }>;
  /**
   ...
   */
  public filteredValue?: Array<string | number | boolean>;
  /**
   * 使用远程过滤
   */
  public filterRemote?: () => void;
  /**
   * 表头分组
   */
...
export declare class TableRenderCreateElementResult {
  ...
  public data?: object;
  public elm?: object;
  public functionalContext?: object;
